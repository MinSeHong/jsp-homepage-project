...
public class FileUtils {
	...
	public static StringBuffer uploadSize(Collection<Part> parts,String saveDirectory) {
		//업로드된 파일 사이즈
		StringBuffer uploadSize=new StringBuffer();
		try {			
			for(Part part:parts) {
				if(part.getContentType() !=null) {//input type="file"인 경우		
					uploadSize.append(Long.toString(part.getSize()/1024)+",");//구분자 ,					
				}				
			}
		}
		catch(Exception e) {//파일 미 첨부시 에러시:null반환			
