...
<body>
    <div class="layout">
        ...
        <c:forEach var="record" items="${cRecords}"> 
            <div class="table-style table-comment-style<c:if test="${requestScope.account eq record.id}"> my-comment </c:if>">
                ...
                <div class="table-column<c:if test="${requestScope.account eq record.id}"> edit</c:if>">${record.bcomment}</div>
                <div class="table-column">${record.postdate}</div>
                <input type="hidden" value="${record.cno}">
            </div>
		</c:forEach>
		
		<c:if test="${empty cRecords}"> 
			<div class="table-style table-comment-style">
		        <div class="empty">no comment</div>
			</div>
		</c:if>
		
        </div>
        <form method="post" action="<c:url value="/bcomment"/>" onsubmit="return Validate()">
            <div class="table-style table-header-style">
                <div class="table-column comment-explain">댓글작성</div>
                <div class="table-column input-style"><input class="comment-input" type="text" maxlength="39" minlength="5" name="bcomment" autocomplete="off"></div>
    ...
    <form name="commentController" method="post" action="<c:url value="/bcomment"/>">
    	<input type="hidden" name="bno" value="${param.bno }">
        <input type="hidden" id="cno" name="cno">
        <input type="hidden" id="bcomment" name="bcomment" value="sdfsdfdsf">
...
<script>
    ...
    var bcomment = document.querySelector('#bcomment');
    ...
    browser.addEventListener("click", function(e){
        ...
        if(e.target.className.includes('table-column edit')){
            ...
            console.log("수정 부분");
        	cno.value = e.target.parentElement.parentElement.children[3].value;
            bcomment.value = e.target.parentElement.parentElement.children[1].children[0].value;
