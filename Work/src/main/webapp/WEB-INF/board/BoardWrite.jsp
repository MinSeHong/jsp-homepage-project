...
<div class="browser">
    <div class="table-style table-body-style">
        <div class="table-column table-column-number"><input class="input-box-style" id="title" type="text" placeholder="Input Title" maxlength="30"></div>
    ...
    <div contenteditable="true" id="contentEditable" class="editor-style textarea-style" spellcheck="false"></div>
</div>
<div class="paging-style">
	<form action="/boardList" method="post" onsubmit="return writeValidate();">
    ...
    <button class="button-box-style" name="out"><a href='<c:url value="/boardList"/><c:if test="${not empty searchText}">&searchColumn=${searchColumn}</c:if><c:if test="${not empty searchText}">&searchText=${searchText}</c:if>'>나가기</a></button>
</div>
