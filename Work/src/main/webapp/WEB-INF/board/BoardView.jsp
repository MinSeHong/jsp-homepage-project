...
<div class="browser">
    ...
    <div class="editor-style textarea-style" spellcheck="false" >${boardRecord.content}</div>
</div>
<div class="paging-style">
	<c:if test="${not empty viewStep.NEXT and viewStep.NEXT!=null}">
		<button class="button-box-style b1" onClick="location.href='/view?bno=${viewStep.NEXT}&nowPage=${param.nowPage}<c:if test="${not empty param.searchText}">&searchColumn=${param.searchColumn}</c:if><c:if test="${not empty param.searchText}">&searchText=${param.searchText}</c:if>'">이전 글</button>
	</c:if>
    <button class="button-box-style b2" onclick="popUpComment()">댓글 목록</button>
    <button class="button-box-style b3" ><a href='<c:url value="/boardList?nowPage=${param.nowPage}"/><c:if test="${not empty param.searchText}">&searchColumn=${param.searchColumn}</c:if><c:if test="${not empty param.searchText}">&searchText=${param.searchText}</c:if>'>나가기</a></button>
    <c:if test="${not empty viewStep.PREV and viewStep.PREV!=null}">
    	<button class="button-box-style b4" onClick="location.href='/view?bno=${viewStep.PREV}&nowPage=${param.nowPage}<c:if test="${not empty param.searchText}">&searchColumn=${param.searchColumn}</c:if><c:if test="${not empty param.searchText}">&searchText=${param.searchText}</c:if>'">다음 글</button>
    </c:if>
    <c:if test="${requestScope.account eq boardRecord.id}">
		...
		<form class="edit-form" action='/edit' method="post">
			...
			<input type="hidden" value="${param.bno}" name="bno"></input>
...
<div class="pop-up-layout-delete">
    <div class="delete-3d-card">
        ...
        <form class="delete-form" action='/boardList' method="post">
            <button class="delete-button" name="SubmitBoardList" value="delete">YES</button>
            <input type="hidden" class="formParameter" name="bno" value="${boardRecord.bno}">
...
<div class="pop-up-layout-comment" onclick="commentButtonClose()">
    <div class="comment-3d-card">
        <iframe src="<c:url value="/bcomment?bno=${param.bno}"/>" id="browserFrame" width="100%" height="95%">
        </iframe>
    </div>
</div>
<script>
